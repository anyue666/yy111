...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private static final int[] DELAY_TO_COLLAPSE_IN_MS = {ALVehicleControlProperty.VEHICLE_PROPERTY_VC_TIRE_WARN_LR, ALExtProperty.VEHICLE_PROPERTY_EXT_VENDOR_REAR_DOOR, 3367, 4717};
    ...
    private void updateSegmentPositions(int i) {
        ...
        for (int i2 = 0; i2 < 4; i2++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_EXPAND_IN_MS[i2], ALVehicleControlProperty.VEHICLE_PROPERTY_VC_TIRE_WARN_LR);
            float[] fArr = this.segmentPositions;
            fArr[1] = fArr[1] + (this.interpolator.getInterpolation(fractionInRange) * 250.0f);
            float fractionInRange2 = getFractionInRange(i, DELAY_TO_COLLAPSE_IN_MS[i2], ALVehicleControlProperty.VEHICLE_PROPERTY_VC_TIRE_WARN_LR);
