...
public class Debug {
    ...
    public static String getState(MotionLayout layout, int stateId, int len) {
        ...
        return (resourceEntryName.length() <= len || (length = resourceEntryName.replaceAll("[^_]", "").length()) <= 0) ? resourceEntryName : resourceEntryName.replaceAll(CharBuffer.allocate((resourceEntryName.length() - len) / length).toString().replace((char) 0, '.') + SystemContact.VOICE_AREA_PARTITION, SystemContact.VOICE_AREA_PARTITION);
    }
