    ...
    private String sideToString(int side) {
        switch (side) {
            ...
            case 4:
                return "bottom";
            case 5:
                return CarUxRestrictionsManager.UX_RESTRICTION_MODE_BASELINE;
            case 6:
                return "start";
            case 7:
                return "end";
    ...
    */
    ...
    public void clone(ConstraintSet set) {
        ...
        for (Integer num : set.mConstraints.keySet()) {
            Constraint constraint = set.mConstraints.get(num);
            if (constraint != null) {
                this.mConstraints.put(num, constraint.m27clone());
            }
        }
    }
    ...
    */
    ...
    static String getDebugName(int v) {
        for (Field field : ConstraintSet.class.getDeclaredFields()) {
            if (field.getName().contains(SystemContact.VOICE_AREA_PARTITION) && field.getType() == Integer.TYPE && Modifier.isStatic(field.getModifiers()) && Modifier.isFinal(field.getModifiers())) {
                try {
